...
def mae_paddle(pred, gt):
    h, w = gt.shape[0:2]
    pred = paddle.cast(pred, dtype='float32')
    gt = paddle.cast(gt, dtype='float32')
    sumError = paddle.sum(paddle.abs(paddle.subtract(pred, gt)))
    maeError = paddle.divide(sumError, paddle.to_tensor(float(h) * float(w) * 255.0 + 1e-4))

    return maeError


def f1score_paddle(pd, gt):
    # gtNum = paddle.sum((gt > 128)* 1)  ## number of ground truth pixels
    gt = paddle.cast(gt, dtype='float32')
    gtNum = (gt > 128).sum()
    pp = pd[gt > 128]
    nn = pd[gt <= 128]

    pp_hist = paddle.histogram(pp, bins=255, min=0, max=255)
    nn_hist = paddle.histogram(nn, bins=255, min=0, max=255)

    pp_hist_flip = paddle.flip(pp_hist, axis=0)
    nn_hist_flip = paddle.flip(nn_hist, axis=0)

    pp_hist_flip_cum = paddle.cumsum(pp_hist_flip, axis=0)
    nn_hist_flip_cum = paddle.cumsum(nn_hist_flip, axis=0)
    precision = pp_hist_flip_cum / (
            pp_hist_flip_cum + nn_hist_flip_cum + 1e-4)  # torch.divide(pp_hist_flip_cum,torch.sum(torch.sum(pp_hist_flip_cum, nn_hist_flip_cum), 1e-4))
    recall = pp_hist_flip_cum / (gtNum + 1e-4)
    f1 = (1 + 0.3) * precision * recall / (0.3 * precision + recall + 1e-4)

    return paddle.reshape(precision, (1, precision.shape[0])), paddle.reshape(recall, (1, recall.shape[0])), \
           paddle.reshape(f1, (1, f1.shape[0]))


def f1_mae_paddle(pred, gt):
    if len(gt.shape) > 2:
        gt = gt[:, :, 0]
    pre, rec, f1 = f1score_paddle(pred, gt)
    mae = mae_paddle(pred, gt)
    return pre.cpu().numpy(), rec.cpu().numpy(), f1.cpu().numpy(), mae.cpu().numpy()
