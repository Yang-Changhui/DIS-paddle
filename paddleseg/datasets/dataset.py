...
class Dataset(paddle.io.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_path_split = label_path.split("/")[-1].split('.')[0][:-3]
        data["ori_gt_path"] = os.path.join(self.dataset_root, 'gt', label_path_split + '.png')
        ...
        div_num = paddle.to_tensor(np.array([[[255.0]], [[255.0]], [[255.0]]]))
        div_num = paddle.cast(div_num, dtype='float32')

        div_num1 = paddle.to_tensor(np.array([[[255.0]]]))
        div_num1 = paddle.cast(div_num1, dtype='float32')
        data['img']  = paddle.divide(paddle.cast(paddle.to_tensor(data['img']), dtype='float32'), div_num)
        data['label'] = paddle.divide(paddle.cast(paddle.to_tensor(data['label']), dtype='float32'), div_num1)
