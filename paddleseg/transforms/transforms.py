...
class Compose:
    ...
    def __call__(self, data):
        ...
        if isinstance(data['img'], str):
            data['img'] = paddle.cast(paddle.load(data['img']),dtype='float32')
        if data['img'] is None:
            raise ValueError('Can\'t read The image file {}!'.format(data[
                                                                         'img']))
        data['img'] = paddle.transpose(data['img'], perm=(1, 2, 0))
        ...
        if self.to_rgb and img_channels == 3:
            data['img'] = cv2.cvtColor(data['img'].numpy(), cv2.COLOR_BGR2RGB)
        if 'label' in data.keys() and isinstance(data['label'], str):
            data['label'] = paddle.to_tensor(paddle.load(data['label'])).numpy()
...
class crop_tensor:
    ...
    def __call__(self, data):
        ...
        if data['img'].shape[2] == 1:
            data['img'] = np.repeat(data['img'], 3, axis=2)
        im_tensor = paddle.to_tensor(data['img'].copy(), dtype='float32')
        im_tensor = paddle.transpose(im_tensor, perm=(2, 0, 1))
        if len(self.size) < 2:
            im_tensor = paddle.cast(im_tensor, dtype='uint8')
        else:
            im_tensor = paddle.unsqueeze(im_tensor, 0)
            im_tensor = F.upsample(im_tensor, self.size, mode="bilinear")
            im_tensor = paddle.squeeze(im_tensor, 0)
            im_tensor = paddle.cast(im_tensor, dtype='uint8')
        data['img'] = im_tensor
        ...
        gt_tensor = paddle.unsqueeze(paddle.to_tensor(data['label'], dtype='uint8'), 0)

        if len(self.size) < 2:
            gt_tensor = paddle.cast(gt_tensor, dtype='uint8')
            return gt_tensor.dtype('uint8')
        else:
            gt_tensor = paddle.unsqueeze(paddle.to_tensor(gt_tensor, dtype='float32'), 0)
            gt_tensor = F.upsample(gt_tensor, self.size, mode="bilinear")
            gt_tensor = paddle.squeeze(gt_tensor, 0)
            gt_tensor = paddle.cast(gt_tensor, dtype='uint8')

        data['label'] = gt_tensor
        return data
...
class GOSRandomHFlip(object):
    ...
    def __call__(self, data):
        ...
        if random.random() >= self.prob:
            data['img'] = paddle.flip(data['img'], axis=[2])
            data['label'] = paddle.flip(data['label'], axis=[2])
...
def im_preprocess(im, size):
    ...
    im_tensor = paddle.to_tensor(im.copy(), dtype='float32')
    im_tensor = paddle.transpose(im_tensor, perm=(2, 0, 1))
    if (len(size) < 2):
        im_tensor = paddle.cast(im_tensor, dtype='uint8')
        return im_tensor
...
def gt_preprocess(gt, size):
    ...
    gt_tensor = paddle.unsqueeze(paddle.to_tensor(gt, dtype='uint8'), 0)
