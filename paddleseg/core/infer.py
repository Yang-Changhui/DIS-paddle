...
def inference(model,
    ...
    pred_val = logits[0][0,:,:,:]
    if len(gt_path) != 0:
        gt = np.squeeze(io.imread(gt_path[0]))  # max = 255
        if gt.max() == 1:
            gt = gt * 255
    else:
        gt = np.zeros((gt.shape[0], gt.shape[1]))
    with paddle.no_grad():
        gt = paddle.to_tensor(gt)
    pred_val = paddle.squeeze(
        F.upsample(paddle.unsqueeze(pred_val, 0), (gt.shape[0],gt.shape[1]), mode='bilinear'))
    ma = paddle.max(pred_val)
    mi = paddle.min(pred_val)
