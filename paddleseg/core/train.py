...
from paddleseg.models.losses import muti_loss_fusion,muti_loss_fusion_kl
...
def train(model,
    ...
    # if interm_sup:
    #     featurenet = get_gt_encoder(model.ISNetGTEncoder(),loader, valid_dataloaders)
    ...
    muti_loss_fusion1=muti_loss_fusion()
    iter = start_iter
    while iter < iters:
        for data in loader:
            iter += 1
            ...
            if precision == 'fp16':
                ...
                if interm_sup:
                    ds, dfs = ddp_model(images) if nranks > 1 else model(images)
                    _, fs = featurenet(labels)  ## extract the gt encodings
                    loss = muti_loss_fusion_kl(ds, labels, dfs, fs, mode='MSE')
                else:
                    ds, _ = ddp_model(images) if nranks > 1 else model(images)
                    loss = muti_loss_fusion1(ds, labels)
            ...
            if (iter % save_interval == 0 or iter == iters) and local_rank == 0:
                ...
                if val_dataset is not None:
                    ...
                    if use_vdl:
                        ...
                        log_writer.add_scalar('Evaluate/mae', mae, iter)
            batch_start = time.time()
